...
export default function PostHarvestForm({ farmerId }: { farmerId: string }) {
  ...
  useEffect(() => {
    const fetchFarmerData = async () => {
      try {
        ...
        toast.error("Something went wrong while fetching farmer details");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Failed to submit crop details");
    }
  };
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-muted-foreground mt-2">Please complete your registration first.</p>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  const Element = isSubmitted ? <PostRequest /> :  <div className="h-[calc(100vh-80px)] overflow-y-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) =>
              setCropDetails({ ...cropDetails, expected_price: e.target.value })
            }
            className="bg-background"
            required
          />
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            onChange={(e) =>
              setCropDetails({ ...cropDetails, description: e.target.value })
            }
            required
          />
        </div>
